...
impl Valley {
    fn new(input: &str) -> Self {
        ...
        Valley { w, h, n_blizz, s_blizz, w_blizz, e_blizz, walls }
    }
    ...
    fn shortest_path(&mut self, start: (usize, usize), goal: (usize, usize)) -> usize {
        ...
        while frontier[goal.1] & (1 << goal.0) == 0 {
            ...
            for c in 0..self.w {
                self.n_blizz[c] = ((self.n_blizz[c] >> 1)
                    | ((self.n_blizz[c] & 2) << (self.h - 3)))
                    & !self.walls[c];
                self.s_blizz[c] = ((self.s_blizz[c] << 1)
                    | ((self.s_blizz[c] >> (self.h - 3)) & 2))
                    & !self.walls[c];
